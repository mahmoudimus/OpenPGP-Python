...
def unarmor(text):
    ...
    for chunk in chunks:
        ...
        if (
            crc24(data)
            != unpack("!L", b"\0" + base64.b64decode(crc[1:].encode("utf-8")))[0]
        ):
            raise OpenPGPException("CRC24 check failed")
...
class Message(object):
    ...
    def signatures(self):
        ...
        for p in msg:
            if isinstance(p, LiteralDataPacket):
                return [
                    (p, list(filter(lambda x: isinstance(x, SignaturePacket), msg)))
                ]
    ...
    def verify_one(self, verifier, sign, sig):
        ...
        if isinstance(sign[0], LiteralDataPacket):
            ...
            isinstance(sign[1], PublicSubkeyPacket)
            or isinstance(sign[1], SecretSubkeyPacket)
        ):
            raw = b"".join(
    ...
    def read(self):
        ...
        if self.version == 3:
            ...
            raise OpenPGPException(
                "Unsupported AsymmetricSessionKeyPacket version: " + self.version
            )
    ...
    def sign_data(self, signers):
        ...
        for mpi in data:
            if (
                ...
                and isinstance(mpi, long)
                or isinstance(mpi, int)
            ):
...
SignaturePacket.subpacket_types[
    32
] = SignaturePacket.EmbeddedSignaturePacket = EmbeddedSignaturePacket
        ...
        for p in message:
            if isinstance(p, SignaturePacket):
                if p.issuer() == keyid16:
                    ...
                    for s in packets:
                        if (
                            isinstance(s, SignaturePacket.EmbeddedSignaturePacket)
    ...
    def read(self):
        ...
        if self.s2k_useage > 0:
            ...
            if chk != checksum(material):
                raise OpenPGPException(
                    "Checksum verification failed when parsing SecretKeyPacket"
                )
