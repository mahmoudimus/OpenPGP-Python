...
try:
    ...
    cryptography_module = True
except ImportError:
    cryptography_module = False


@unittest.skipIf(cryptography_module is False, "cryptography module not present")
class TestMessageVerification(unittest.TestCase):
    ...
    def testSigningMessagesRSA(self):
        ...
        self.assertEqual(sign.verify(reparsedM), reparsedM.signatures())
    ...
    #    reparsedM = OpenPGP.Message.parse(m)
    #    self.assertEqual(OpenPGP.cryptography.Wrapper(wkey).verify(reparsedM), reparsedM.signatures())
    #
...
class TestKeyVerification(unittest.TestCase):
    ...
    def testSigningKeysRSA(self):
        ...
        self.assertEqual(wkey.verify(reparsedM), reparsedM.signatures())
...
class TestDecryption(unittest.TestCase):
    ...
    def testDecryptSecretKey(self):
        key = OpenPGP.Message.parse(
            open(
                os.path.dirname(__file__) + "/data/encryptedSecretKey.gpg", "rb"
            ).read()
        )
        skey = OpenPGP.cryptography.Wrapper(key[0]).decrypt_secret_key("hello")
        self.assertEqual(not (not skey), True)
...
class TestEncryption(unittest.TestCase):
    def testEncryptSymmetric(self):
        data = OpenPGP.LiteralDataPacket("This is text.", "u", "stuff.txt")
        encrypted = OpenPGP.cryptography.Wrapper(OpenPGP.Message([data])).encrypt(
            "secret"
        )
        decrypted = OpenPGP.cryptography.Wrapper(encrypted).decrypt_symmetric("secret")
        self.assertEqual(decrypted[0].data, b"This is text.")
